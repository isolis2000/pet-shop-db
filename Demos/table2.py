...
import operator
from DatabaseRead import readDB
...
data = readDB()
MAX_ROWS, MAX_COLS, COL_HEADINGS = (
    len(data),
    len(data[0]),
    (
        "Producto",
...
layout = (
    [[sg.Text("Click on a column header to sort by that column", font="Default 16")]]
    + [
        [sg.Text(" " * 15)]
        + [
            sg.Text(s, key=s, enable_events=True, font="Courier 12", size=(8, 2))
            for i, s in enumerate(COL_HEADINGS)
        ]
    ]
    + [
        [sg.T(r, size=(4, 1))]
...
while True:  # Event Loop
    ...
    if event.startswith("Down"):
        ...
        try:
            table = [
                [int(values[(row, col)]) for col in range(MAX_COLS)]
            ...
            sg.popup_error(
                "Error in table",
                "Your table must contain only ints if you wish to sort by column",
            )
        else:
            ...
            ]  # make all column headings be normal fonts
    ...
    if event.startswith("Show Table"):
        table = [
            [values[(row, col)] for col in range(MAX_COLS)] for row in range(MAX_ROWS)
        ]
        sg.popup_scrolled(
            "your_table = [ ",
