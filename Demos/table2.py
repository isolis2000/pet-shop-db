...
data = readDB()
MAX_ROWS, MAX_COLS, COL_HEADINGS = len(data), len(data[0]), ('Producto', 'Precio', 'Ganancia %', 'Ganancia', 'Codigo de Barras',)
...
layout = [[sg.Text('Click on a column header to sort by that column', font='Default 16')]] + \
         [[sg.Text(' ' * 15)] + [sg.Text(s, key=s, enable_events=True, font='Courier 12', size=(8, 2)) for i, s in enumerate(COL_HEADINGS)]] + \
